              ...
              >
                <div className="relative">
                  ...
                  <div className="absolute top-2 left-2 z-10 flex flex-col gap-1 items-start pointer-events-none">
                    {art.isOnlineOnly ? (
                      <span className="bg-red-50 text-red-700 border border-red-200 text-[7px] font-mono font-bold tracking-widest px-1.5 py-0.5 uppercase shadow-sm">
                        {lang === 'FR' ? 'Vente En Ligne' : 'Online Order Only'}
                      </span>
                    ) : (
                      <span className="bg-emerald-50 text-emerald-800 border border-emerald-100 text-[7px] font-mono tracking-widest font-bold px-1.5 py-0.5 uppercase shadow-sm">
                        {lang === 'FR' ? 'En Galerie' : 'In Gallery'}
                      </span>
                    )}

                    {art.onExhibit && (
                      <span className="bg-neutral-100 text-neutral-800 border border-black/15 text-[7px] font-mono tracking-widest font-bold px-1.5 py-0.5 uppercase shadow-sm">
                        {lang === 'FR' ? 'Exposition Permanente' : 'Permanent Exhibit'}
                      </span>
                    )}
                  </div>

                  <div className="absolute inset-0 z-10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/80 backdrop-blur-[1px] pointer-events-none">
                    <div className="flex items-center space-x-1.5 border border-black/30 text-black bg-white px-3 py-2 text-[9px] tracking-widest uppercase font-mono font-bold">
                ...
                {/* Info Block */}
                <div className="p-3 flex flex-col justify-between flex-grow self-stretch">
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[9px] font-mono tracking-widest text-neutral-500 uppercase font-medium truncate">
                    ...
                    <h3 
                      onClick={() => onSelectArtwork(art)}
                      className="font-serif text-sm font-light tracking-wide text-neutral-900 group-hover:text-black transition-colors cursor-pointer line-clamp-2 leading-snug"
                    >
                      {art.title}
                    </h3>

                    <p className="text-[10px] font-serif italic text-neutral-800 tracking-wide font-semibold uppercase line-clamp-1">
                      {art.artistName}
                    </p>
                  </div>

                  <div className="pt-2.5 mt-2.5 border-t border-black/10 flex items-center justify-between gap-2">
                    <span className="text-[9px] font-mono font-bold text-neutral-900 uppercase tracking-wider truncate">
                      {art.isOnlineOnly ? (
                    ...
                    <button
                      onClick={() => onSelectArtwork(art)}
                      className="text-[9px] tracking-widest font-mono font-bold text-black hover:opacity-60 flex items-center shrink-0 transition-colors uppercase cursor-pointer"
